//===-- SISCISelDAGToDAG.cpp - A dag to dag inst selector for SISC ------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines an instruction selector for the SISC target.
//
//===----------------------------------------------------------------------===//

#include "SISCISelDAGToDAG.h"
...
void SISCDAGToDAGISel::PostprocessISelDAG() {
...
static SDNode *selectImm(SelectionDAG *CurDAG, const SDLoc &DL, int64_t Imm,
  ...
  SISCMatInt::generateInstSeq(Imm, XLenVT == MVT::i64, Seq);

  SDNode *Result = nullptr;
  SDValue SrcReg = CurDAG->getRegister(SISC::X0, XLenVT);
  for (SISCMatInt::Inst &Inst : Seq) {
    SDValue SDImm = CurDAG->getTargetConstant(Inst.Imm, DL, XLenVT);
    if (Inst.Opc == SISC::LUI)
      Result = CurDAG->getMachineNode(SISC::LUI, DL, XLenVT, SDImm);
    else
...
void SISCDAGToDAGISel::Select(SDNode *Node) {
  ...
  case ISD::ADD: {
    ...
    if (auto *ConstOp = dyn_cast<ConstantSDNode>(Node->getOperand(1))) {
      ...
      auto *NodeAddi0 = CurDAG->getMachineNode(SISC::ADDI, DL, VT,
                                               Node->getOperand(0), ImmOp0);
      auto *NodeAddi1 = CurDAG->getMachineNode(SISC::ADDI, DL, VT,
  ...
  case ISD::FrameIndex: {
    ...
    ReplaceNode(Node, CurDAG->getMachineNode(SISC::ADDI, DL, VT, TFI, Imm));
    return;
  }
  case ISD::SRL: {
    ...
    if (Op1.getOpcode() == ISD::Constant &&
      ...
      if ((Mask | maskTrailingOnes<uint64_t>(ShAmt)) == 0xffffffff) {
        ...
        CurDAG->SelectNodeTo(Node, SISC::SRLIW, XLenVT, Op0.getOperand(0),
                             ShAmtVal);
        return;
      }
    }
  ...
  case SISCISD::READ_CYCLE_WIDE:
    assert(!Subtarget->is64Bit() && "READ_CYCLE_WIDE is only used on sisc32");

    ReplaceNode(Node, CurDAG->getMachineNode(SISC::ReadCycleWide, DL, MVT::i32,
                                             MVT::i32, MVT::Other,
                                             Node->getOperand(0)));
    return;
  }
...
bool SISCDAGToDAGISel::SelectInlineAsmMemoryOperand(
...
bool SISCDAGToDAGISel::SelectAddrFI(SDValue Addr, SDValue &Base) {
...
bool SISCDAGToDAGISel::SelectSLOI(SDValue N, SDValue &RS1, SDValue &Shamt) {
  MVT XLenVT = Subtarget->getXLenVT();
...
bool SISCDAGToDAGISel::SelectSROI(SDValue N, SDValue &RS1, SDValue &Shamt) {
  MVT XLenVT = Subtarget->getXLenVT();
  if (N.getOpcode() == ISD::OR) {
    SDValue Or = N;
...
bool SISCDAGToDAGISel::SelectRORI(SDValue N, SDValue &RS1, SDValue &Shamt) {
  MVT XLenVT = Subtarget->getXLenVT();
...
bool SISCDAGToDAGISel::SelectSLLIUW(SDValue N, SDValue &RS1, SDValue &Shamt) {
...
bool SISCDAGToDAGISel::SelectSLOIW(SDValue N, SDValue &RS1, SDValue &Shamt) {
...
bool SISCDAGToDAGISel::SelectSROIW(SDValue N, SDValue &RS1, SDValue &Shamt) {
...
bool SISCDAGToDAGISel::SelectRORIW(SDValue N, SDValue &RS1, SDValue &Shamt) {
...
bool SISCDAGToDAGISel::SelectFSRIW(SDValue N, SDValue &RS1, SDValue &RS2,
                                    SDValue &Shamt) {
  if (N.getOpcode() == ISD::SIGN_EXTEND_INREG &&
      Subtarget->getXLenVT() == MVT::i64 &&
      cast<VTSDNode>(N.getOperand(1))->getVT() == MVT::i32) {
...
void SISCDAGToDAGISel::doPeepholeLoadStoreADDI() {
  ...
  while (Position != CurDAG->allnodes_begin()) {
    ...
    if (!Base.isMachineOpcode() || Base.getMachineOpcode() != SISC::ADDI)
      continue;
...
// This pass converts a legalized DAG into a SISC-specific DAG, ready
// for instruction scheduling.
FunctionPass *llvm::createSISCISelDag(SISCTargetMachine &TM) {
  return new SISCDAGToDAGISel(TM);
}
