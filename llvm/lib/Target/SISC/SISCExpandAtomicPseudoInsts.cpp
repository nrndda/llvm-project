//===-- SISCExpandAtomicPseudoInsts.cpp - Expand atomic pseudo instrs. ---===//
//
...
#define SISC_EXPAND_ATOMIC_PSEUDO_NAME                                        \
  "SISC atomic pseudo instruction expansion pass"

namespace {

class SISCExpandAtomicPseudo : public MachineFunctionPass {
public:
  const SISCInstrInfo *TII;
  static char ID;

  SISCExpandAtomicPseudo() : MachineFunctionPass(ID) {
    initializeSISCExpandAtomicPseudoPass(*PassRegistry::getPassRegistry());
  }

  bool runOnMachineFunction(MachineFunction &MF) override;

  StringRef getPassName() const override {
    return SISC_EXPAND_ATOMIC_PSEUDO_NAME;
  }
...
char SISCExpandAtomicPseudo::ID = 0;

bool SISCExpandAtomicPseudo::runOnMachineFunction(MachineFunction &MF) {
  TII = static_cast<const SISCInstrInfo *>(MF.getSubtarget().getInstrInfo());
  bool Modified = false;
  for (auto &MBB : MF)
...
bool SISCExpandAtomicPseudo::expandMBB(MachineBasicBlock &MBB) {
  bool Modified = false;
...
bool SISCExpandAtomicPseudo::expandMI(MachineBasicBlock &MBB,
                                       MachineBasicBlock::iterator MBBI,
                                       MachineBasicBlock::iterator &NextMBBI) {
...
static void doAtomicBinOpExpansion(const SISCInstrInfo *TII, MachineInstr &MI,
  ...
  case AtomicRMWInst::Nand:
    BuildMI(LoopMBB, DL, TII->get(SISC::AND), ScratchReg)
        .addReg(DestReg)
        .addReg(IncrReg);
    BuildMI(LoopMBB, DL, TII->get(SISC::XORI), ScratchReg)
        .addReg(ScratchReg)
        .addImm(-1);
    break;
  }
...
static void insertMaskedMerge(const SISCInstrInfo *TII, DebugLoc DL,
  ...
  BuildMI(MBB, DL, TII->get(SISC::XOR), ScratchReg)
      .addReg(OldValReg)
      .addReg(NewValReg);
  BuildMI(MBB, DL, TII->get(SISC::AND), ScratchReg)
      .addReg(ScratchReg)
      .addReg(MaskReg);
  BuildMI(MBB, DL, TII->get(SISC::XOR), DestReg)
      .addReg(OldValReg)
      .addReg(ScratchReg);
}

static void doMaskedAtomicBinOpExpansion(
    const SISCInstrInfo *TII, MachineInstr &MI, DebugLoc DL,
  ...
  case AtomicRMWInst::Xchg:
    BuildMI(LoopMBB, DL, TII->get(SISC::ADDI), ScratchReg)
        .addReg(IncrReg)
        .addImm(0);
    break;
  case AtomicRMWInst::Add:
    BuildMI(LoopMBB, DL, TII->get(SISC::ADD), ScratchReg)
        .addReg(DestReg)
        .addReg(IncrReg);
    break;
  case AtomicRMWInst::Sub:
    BuildMI(LoopMBB, DL, TII->get(SISC::SUB), ScratchReg)
        .addReg(DestReg)
        .addReg(IncrReg);
    break;
  case AtomicRMWInst::Nand:
...
bool SISCExpandAtomicPseudo::expandAtomicBinOp(
...
static void insertSext(const SISCInstrInfo *TII, DebugLoc DL,
                       MachineBasicBlock *MBB, Register ValReg,
                       Register ShamtReg) {
  BuildMI(MBB, DL, TII->get(SISC::SLL), ValReg)
      .addReg(ValReg)
      .addReg(ShamtReg);
  BuildMI(MBB, DL, TII->get(SISC::SRA), ValReg)
      .addReg(ValReg)
      .addReg(ShamtReg);
}

bool SISCExpandAtomicPseudo::expandAtomicMinMaxOp(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator MBBI,
    AtomicRMWInst::BinOp BinOp, bool IsMasked, int Width,
    MachineBasicBlock::iterator &NextMBBI) {
  ...
  BuildMI(LoopHeadMBB, DL, TII->get(getLRForRMW32(Ordering)), DestReg)
      .addReg(AddrReg);
  BuildMI(LoopHeadMBB, DL, TII->get(SISC::AND), Scratch2Reg)
      .addReg(DestReg)
      .addReg(MaskReg);
  BuildMI(LoopHeadMBB, DL, TII->get(SISC::ADDI), Scratch1Reg)
      .addReg(DestReg)
      .addImm(0);
  ...
  case AtomicRMWInst::Max: {
    ...
    BuildMI(LoopHeadMBB, DL, TII->get(SISC::BGE))
        .addReg(Scratch2Reg)
        .addReg(IncrReg)
        .addMBB(LoopTailMBB);
    break;
  ...
  case AtomicRMWInst::UMax:
    BuildMI(LoopHeadMBB, DL, TII->get(SISC::BGEU))
        .addReg(Scratch2Reg)
        .addReg(IncrReg)
        .addMBB(LoopTailMBB);
    break;
  ...
  BuildMI(LoopTailMBB, DL, TII->get(getSCForRMW32(Ordering)), Scratch1Reg)
      .addReg(AddrReg)
      .addReg(Scratch1Reg);
  BuildMI(LoopTailMBB, DL, TII->get(SISC::BNE))
      .addReg(Scratch1Reg)
      .addReg(SISC::X0)
      .addMBB(LoopHeadMBB);
...
bool SISCExpandAtomicPseudo::expandAtomicCmpXchg(
  ...
  if (!IsMasked) {
    ...
    BuildMI(LoopHeadMBB, DL, TII->get(getLRForRMW(Ordering, Width)), DestReg)
        .addReg(AddrReg);
    BuildMI(LoopHeadMBB, DL, TII->get(SISC::BNE))
        .addReg(DestReg)
        .addReg(CmpValReg)
        .addMBB(DoneMBB);
    // .looptail:
    ...
    BuildMI(LoopHeadMBB, DL, TII->get(getLRForRMW(Ordering, Width)), DestReg)
        .addReg(AddrReg);
    BuildMI(LoopHeadMBB, DL, TII->get(SISC::AND), ScratchReg)
        .addReg(DestReg)
        .addReg(MaskReg);
    BuildMI(LoopHeadMBB, DL, TII->get(SISC::BNE))
        .addReg(ScratchReg)
...
INITIALIZE_PASS(SISCExpandAtomicPseudo, "sisc-expand-atomic-pseudo",
                SISC_EXPAND_ATOMIC_PSEUDO_NAME, false, false)

namespace llvm {

FunctionPass *createSISCExpandAtomicPseudoPass() {
  return new SISCExpandAtomicPseudo();
}
