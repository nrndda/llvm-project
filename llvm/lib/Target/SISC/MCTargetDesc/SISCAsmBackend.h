//===-- SISCAsmBackend.h - SISC Assembler Backend -----------------------===//
//
...
#ifndef LLVM_LIB_TARGET_SISC_MCTARGETDESC_SISCASMBACKEND_H
#define LLVM_LIB_TARGET_SISC_MCTARGETDESC_SISCASMBACKEND_H
...
class SISCAsmBackend : public MCAsmBackend {
...
public:
  SISCAsmBackend(const MCSubtargetInfo &STI, uint8_t OSABI, bool Is64Bit,
                  const MCTargetOptions &Options)
      : MCAsmBackend(support::little), STI(STI), OSABI(OSABI), Is64Bit(Is64Bit),
        TargetOptions(Options) {
    TargetABI = SISCABI::computeTargetABI(
        STI.getTargetTriple(), STI.getFeatureBits(), Options.getABIName());
    SISCFeatures::validate(STI.getTargetTriple(), STI.getFeatureBits());
  }
  ~SISCAsmBackend() override {}
  ...
  bool willForceRelocations() const {
    return ForceRelocs || STI.getFeatureBits()[SISC::FeatureRelax];
  }
  ...
  unsigned getNumFixupKinds() const override {
    return SISC::NumTargetFixupKinds;
  }
