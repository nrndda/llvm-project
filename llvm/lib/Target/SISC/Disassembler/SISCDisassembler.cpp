//===-- SISCDisassembler.cpp - Disassembler for SISC --------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the SISCDisassembler class.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "sisc-disassembler"

typedef MCDisassembler::DecodeStatus DecodeStatus;

namespace {
class SISCDisassembler : public MCDisassembler {
  std::unique_ptr<MCInstrInfo const> const MCII;

public:
  SISCDisassembler(const MCSubtargetInfo &STI, MCContext &Ctx,
...
static MCDisassembler *createSISCDisassembler(const Target &T,
                                               const MCSubtargetInfo &STI,
                                               MCContext &Ctx) {
  return new SISCDisassembler(STI, Ctx, T.createMCInstrInfo());
}

extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeSISCDisassembler() {
  // Register the disassembler for each target.
  TargetRegistry::RegisterMCDisassembler(getTheSISC32Target(),
                                         createSISCDisassembler);
  TargetRegistry::RegisterMCDisassembler(getTheSISC64Target(),
                                         createSISCDisassembler);
}
...
static DecodeStatus DecodeGPRRegisterClass(MCInst &Inst, uint64_t RegNo,
  ...
  const FeatureBitset &FeatureBits =
      static_cast<const MCDisassembler *>(Decoder)
          ->getSubtargetInfo()
          .getFeatureBits();
  bool IsRV32E = FeatureBits[SISC::FeatureRV32E];

  if (RegNo >= 32 || (IsRV32E && RegNo >= 16))
    return MCDisassembler::Fail;

  Register Reg = SISC::X0 + RegNo;
...
static void addImplySP(MCInst &Inst, int64_t Address, const void *Decoder) {
  if (Inst.getOpcode() == SISC::C_LWSP || Inst.getOpcode() == SISC::C_SWSP ||
      Inst.getOpcode() == SISC::C_LDSP || Inst.getOpcode() == SISC::C_SDSP ||
      Inst.getOpcode() == SISC::C_FLWSP ||
      Inst.getOpcode() == SISC::C_FSWSP ||
      Inst.getOpcode() == SISC::C_FLDSP ||
      Inst.getOpcode() == SISC::C_FSDSP ||
      Inst.getOpcode() == SISC::C_ADDI4SPN) {
    DecodeGPRRegisterClass(Inst, 2, Address, Decoder);
  }
  if (Inst.getOpcode() == SISC::C_ADDI16SP) {
    DecodeGPRRegisterClass(Inst, 2, Address, Decoder);
    DecodeGPRRegisterClass(Inst, 2, Address, Decoder);
  }
}
...
static DecodeStatus decodeFRMArg(MCInst &Inst, uint64_t Imm,
  ...
  if (!llvm::SISCFPRndMode::isValidRoundingMode(Imm))
    return MCDisassembler::Fail;
...
#include "SISCGenDisassemblerTables.inc"
...
DecodeStatus SISCDisassembler::getInstruction(MCInst &MI, uint64_t &Size,
  ...
  if ((Bytes[0] & 0x3) == 0x3) {
    ...
    LLVM_DEBUG(dbgs() << "Trying SISC32 table :\n");
    ...
    if (!STI.getFeatureBits()[SISC::Feature64Bit]) {
      LLVM_DEBUG(
          dbgs() << "Trying SISC32Only_16 table (16-bit Instruction):\n");
      // Calling the auto-generated decoder function.
      Result = decodeInstruction(DecoderTableSISC32Only_16, MI, Insn, Address,
                                 this, STI);
    ...
    if (STI.getFeatureBits()[SISC::FeatureExtZbproposedc] &&
        STI.getFeatureBits()[SISC::FeatureStdExtC]) {
      LLVM_DEBUG(
    ...
    LLVM_DEBUG(dbgs() << "Trying SISC_C table (16-bit Instruction):\n");
    // Calling the auto-generated decoder function.
