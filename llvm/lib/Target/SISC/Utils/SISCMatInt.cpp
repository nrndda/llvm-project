//===- SISCMatInt.cpp - Immediate materialisation -------------*- C++ -*--===//
//
...
void generateInstSeq(int64_t Val, bool IsRV64, InstSeq &Res) {
  if (isInt<32>(Val)) {
    ...
    if (Hi20)
      Res.push_back(Inst(SISC::LUI, Hi20));

    if (Lo12 || Hi20 == 0) {
      unsigned AddiOpc = (IsRV64 && Hi20) ? SISC::ADDIW : SISC::ADDI;
  ...
  Res.push_back(Inst(SISC::SLLI, ShiftAmount));
  if (Lo12)
    Res.push_back(Inst(SISC::ADDI, Lo12));
}
